...
void OfenHUD::onInit() {
    ...
    titelLabel->setText("Furnace", 36.0f);
    titelLabel->color = glm::vec4(1.0f, 0.5f, 0.2f, 1.0f);
    container->addChild(titelLabel);

    rezepte = GegenstandDaten::getInstance().getRezepteFuer(RezeptQuelle::OFEN);
...
void OfenHUD::onUpdate(GLFWwindow* window, float deltaTime, Transform& cameraTransform) {
    if (Input::isKeyJustPressed(GLFW_KEY_R)) {
        if (offen) {
            umschalten(window);
        } else if (!inventarOffen && spielerNaheOfen()) {
            umschalten(window);
        }
    }
...
bool OfenHUD::spielerNaheOfen() const {
    const auto& spieler = getNodesInGroup("spielerInsel");
    const auto& oefen = getNodesInGroup("ofen");
    if (spieler.empty() || oefen.empty()) return false;

    glm::vec3 spielerPos = spieler[0]->getGlobalTransform().position;
    for (auto* o : oefen) {
        if (glm::distance(spielerPos, o->getGlobalTransform().position) < 5.0f) {
            return true;
        }
    }
    return false;
...
void OfenHUD::umschalten(GLFWwindow* window) {
    offen = !offen;
    inventarOffen = offen;
...
void OfenHUD::herstellen() {
    ...
    std::cout << "[Furnace] Crafted: " << info.name << " x" << r.ausgabeAnzahl << std::endl;
    QuestManager::getInstance().progressObjective("craft_" + info.name, 1);
}

void OfenHUD::scrollAktualisieren() {
